...
class FirebaseRepository @Inject constructor(@ApplicationContext private val context: Context) {
    ...
    suspend fun syncAll(): Int = withContext(Dispatchers.IO) {
        var count = 0
        try {
            getSnapshot("users")?.children?.forEach { try { val m = it.value as? Map<*,*>; if (m != null) { val u = UserRecord(code = m["code"] as? String ?: "", role = m["role"] as? String ?: "", name = m["name"] as? String ?: "", schoolCode = m["schoolCode"] as? String ?: "", active = m["active"] as? Boolean ?: true); if (u.code.isNotEmpty()) { saveLocalUser(u); count++ } } } catch (_: Exception) { } }
            getSnapshot("schools")?.children?.forEach { try { val m = it.value as? Map<*,*>; if (m != null) { val s = SchoolRecord(code = m["code"] as? String ?: "", name = m["name"] as? String ?: "", adminCode = m["adminCode"] as? String ?: "", directorName = m["directorName"] as? String ?: "", teacherCount = (m["teacherCount"] as? Long)?.toInt() ?: 0, teacherCodes = (m["teacherCodes"] as? List<*>)?.mapNotNull { it as? String } ?: emptyList()); if (s.code.isNotEmpty()) { saveLocalSchool(s); count++ } } } catch (_: Exception) { } }
            getSnapshot("games")?.children?.forEach { try { val m = it.value as? Map<*,*>; if (m != null) { val g = GameRecord(title = m["title"] as? String ?: "", description = m["description"] as? String ?: "", category = m["category"] as? String ?: "", fileName = m["fileName"] as? String ?: ""); if (g.fileName.isNotEmpty()) { saveLocalGame(g); count++ } } } catch (_: Exception) { } }
        } catch (e: Exception) { e.printStackTrace() }; count
    }
